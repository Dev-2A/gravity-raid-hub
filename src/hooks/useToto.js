...
export function useToto() {
  ...
  const placeBet = async (memberId, betValue) => {
    if (!selectedRound)
      return { data: null, error: "진행 중인 라운드가 없습니다" };
  ...
  // 결과 입력 (범용) + 업적 체크
  const finishRound = async (resultValue, checkAchievementsFn = null) => {
    if (!selectedRound) return { error: "진행 중인 라운드가 없습니다" };
    ...
    try {
      ...
      // 업적 체크: 해당 라운드에 베팅한 모든 멤버
      if (checkAchievementsFn && bets.length > 0) {
        const newAchievements = [];
        for (const bet of bets) {
          const achieved = await checkAchievementsFn(bet.member_id);
        ...
        return { error: null, newAchievements };
      }
